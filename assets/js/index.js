$('#login').submit(function (e) {
  ...
  $.ajax({
    ...
    data: $(this).serialize(),
    beforeSend: function () {
      document.querySelector('#tblmasuk').innerHTML =
    ...
    success: function (data) {
      if (data.status === 'konfirmasi') {
        $('.login-danger').fadeIn();
        $('.login-danger').html(
          'Gagal! Anda belum melakukan aktivasi pendaftaran'
        );
        setTimeout(function () {
...
function reset_pass(username, email) {
  $.ajax({
    type: 'post',
    dataType: 'JSON',
    url: 'proses/reset',
...
$('#daftar').submit(function (e) {
  ...
  $.ajax({
    ...
    data: $(this).serialize(),
    beforeSend: function () {
      document.querySelector('#tbldaftar').innerHTML =
        '<i class="mdi mdi-spin mdi-loading align-middle ms-1"></i> Loading...';
    },
    success: function (data) {
      if (data.status == 'alert_captcha') {
        ...
        $('.alert-user').fadeIn();
        $('.alert-user').html(
          'Username "' + data.user + '" sudah ada yang gunakan'
        );
        setTimeout(function () {
          $('.alert-user').fadeOut();
        }, 5000);
        ...
        $('.alert-email').fadeIn();
        $('.alert-email').html(
          'email "' + data.email + '" ini sudah terdaftar'
        );
        setTimeout(function () {
          $('.alert-email').fadeOut();
        }, 5000);
    ...
    complete: function (data) {
      document.querySelector('#tbldaftar').innerHTML =
        '<i class="ri-save-2-fill me-1 align-middle"></i> Ajukan Pendaftaran';
    },
  });
});
...
$('#reset').submit(function (e) {
  ...
  $.ajax({
    ...
    data: $(this).serialize(),
    beforeSend: function () {
      document.querySelector('#tblreset').innerHTML =
        '<i class="mdi mdi-spin mdi-loading align-middle ms-1"></i> Loading...';
    },
    success: function (data) {
      if (data.status === 'alert_email') {
        $('.reset-danger').fadeIn();
        $('.reset-danger').html('Gagal! email ini tidak terdaftar!');
        setTimeout(function () {
          $('.reset-danger').fadeOut();
        }, 2000); //will call the function after 2 secs.
      } else if (data.status == 'sukses') {
        $('.reset-danger').fadeOut();
        $('.reset-success').fadeIn();
        $('.reset-success').html(
          'Reset Password Berhasil!<br>kami telah mengirimkan informasi ke email "' +
            data.email +
            '"'
        );
        $('#email').val('');
        reset_pass(data.username, data.email);
      }
    },
    complete: function (data) {
      document.querySelector('#tblreset').innerHTML = 'Reset Password';
    },
  });
});
