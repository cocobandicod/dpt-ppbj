    ...
    <li class="list-group-item ps-0">
        <div class="d-flex align-items-start">
            ...
            <div class="flex-shrink-0 ms-2 pe-3">
                ...
                ?>
                    <a href="<?= $url; ?>ppk/daftar-paket/<?= $_POST['id']; ?>/<?= $_POST['tahun']; ?>/rincian-hps" class="btn btn-success btn-sm"><i class="ri-upload-cloud-2-line"></i> Upload Rincian HPS</a>
                ...
                ?>
                    ...
                    <a href="<?= $url; ?>ppk/daftar-paket/<?= $_POST['id']; ?>/<?= $_POST['tahun']; ?>/rincian-hps" class="btn btn-success btn-sm"><i class="ri-upload-cloud-2-line"></i> Ubah Rincian HPS</a>
...
?>
    <a href="<?= $url; ?>template/template-hps.xlsx" class="btn btn-success btn-sm"><i class="ri-file-download-line"></i> Download Template HPS</a>
    <button class="btn btn-success btn-sm" data-bs-toggle="modal" data-bs-target="#DetailModal" data-id="<?= $_POST['id']; ?>" data-act="form_hps"><i class="ri-upload-cloud-2-line"></i> Upload Rincian HPS</button>
    <div class="alert alert-info mt-3" role="alert">
        <p><b>Petunjuk Upload Rincian HPS</b></p>
        <ul>
            <li>Silahkan upload rincian HPS menggunakan Template Rincian HPS yang sudah di download; atau</li>
            <li>Upload rincian HPS yang sudah disesuaikan penamaan kolomnya dan menggunakan file yang memiliki extensi <b>*.xls</b></li>
        </ul>
        <p><b>Petunjuk Upload Rincian HPS</b></p>
        <ul>
            <li>Jangan mengubah nama kolom, menghapus kolom atau menambah kolom;</li>
            <li>Jangan mengubah nama sheet; dan</li>
            <li>Gunakan tanda 'koma' sebagai pemisah desimal.</li>
        </ul>
    </div>
    ...
    <form id="form2">
        <input type="hidden" name="act" value="simpan_hps">
        <input type="hidden" name="id" value="<?= $_POST['id']; ?>">
        <input type="hidden" name="hps" value="<?= $totalharga; ?>">
        <button type="submit" class="btn btn-success" id="simpan2"><i class="ri-save-line"></i> Simpan</button>
    </form>
<?php }
...
?>
    <ul class="list-group list-group-flush border-dashed px-0 pe-0">
        ...
        <li class="list-group-item ps-0 pe-3">
            <div class="d-flex align-items-start">
                <div class="flex-grow-1 pt-0 ms-3">
                    <a href="#" class="text-decoration-underline" data-bs-toggle="modal" data-bs-target="#DetailModal" data-id="<?= $_POST['id']; ?>" data-act="form_kontrak">Rancangan Kontrak</a> *
                </div>
        ...
        <div class="modal-body pt-3">
            <div class="row">
                <div class="col-xxl-12 col-md-12 pt-2">
                    ...
                    <input type="file" onchange="return validasiFile()" accept=".xls, .xlsx" name="fupload" id="file" class="form-control" <?= $required; ?>>
                    ...
                    <tbody>
                        ...
                        ?>
                                <tr>
                                    ...
                                    <td class="P-2 text-center"><a href="#" id="del" data-id="<?= @$row['id']; ?>" data-nama="<?= @$row['nama_file']; ?>" data-act="delfile"><i class="ri-delete-bin-fill align-bottom text-danger fs-15"></i></a></td>
                                </tr>
                        <?php $no++;
                            }
                        } ?>
                ...
                <table class="table table-bordered table-sm mt-3">
                    <thead class="table-light">
                        <tr>
                            ...
                            <th class="P-2">Nama File</th>
                            <th class="P-2">Tanggal Upload</th>
                            <th class="P-2 text-center" style="width: 5%;">Aksi</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php
...
if (@$_POST['judul'] == 'ukpbj') {
    $cek = $proses->cek_count('penugasan a LEFT JOIN paket_pekerjaan b ON a.kode_paket = b.kode_paket', 'b.kode_paket = "' . $_POST['id'] . '" AND a.status = "UKPBJ"');
    if ($cek == 0) {
        ...
        <table class="table table-sm fs-13 mb-0">
            ...
            <tbody>
                ...
                $row = $proses->tampil_data_saja(
                    ...
                    '1=1 AND a.kode_paket = "' . $_POST['id'] . '" AND a.status = "UKPBJ"'
                );
                ?>
                <?php if (empty($row)) { ?>
                    <tr>
                ...
                <?php } else { ?>
                    <tr>
                        ...
                        <td class="text-center p-2"><a href="#" id="hapus_ukpbj" data-nama="<?= @$row['nama']; ?>" data-id="<?= @$row['id']; ?>"><i class="ri-delete-bin-fill text-danger fs-15"></i></a></td>
                    </tr>
                <?php } ?>
            </tbody>
        </table>
...
if (@$_POST['judul'] == 'kirim_ukpbj') {
    $cek = $proses->tampil_data_saja('*', 'undangan_ukpbj', '1=1 AND kode_paket = "' . $_POST['id'] . '" ORDER BY timestamp DESC');
    if (empty($cek['id'])) {
        echo '<a href="#" class="btn btn-warning btn-sm mb-0" data-bs-toggle="modal" data-bs-target="#DetailModal" data-id="' . $_POST['id'] . '" data-act="email_ukpbj"> <i class="ri-mail-send-line"></i> Kirim Email UKPBJ</a>';
    } else {
        $kirimulang = '<a href="#" class="btn btn-warning btn-sm mb-0" data-bs-toggle="modal" data-bs-target="#DetailModal" data-id="' . $_POST['id'] . '" data-act="email_ukpbj"> <i class="ri-mail-send-line"></i> Kirim Ulang</a>';
...
?>
    ...
    <form id="kirim_email" enctype="multipart/form-data">
        <?php
        echo '<input type="hidden" name="act" value="kirim_email_ukpbj">';
        ...
        <p>Kami informasikan bahwa anda telah di tunjuk oleh PPK <?= $_SESSION['nama']; ?> pada paket pekerjaan</p>
                <table>
                        <tr>
                            <td >Kode Paket</td>
                            <td ><?= @$s['kode_paket']; ?></td>
