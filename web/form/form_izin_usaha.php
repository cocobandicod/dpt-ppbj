...
cek_url($url, $proses, $_GET['act'], 'izin_usaha', 'id_profil ="' . @$_SESSION['kode_profil'] . '" AND id ="' . @$_GET['id'] . '"');
?>
<!doctype html>
...
<body>
                        ...
                        <div class="card card-height-100">
                            ...
                            <div class="card-body">
                                <form id="form" enctype="multipart/form-data">
                                    <?php
                                    if ($_GET['act'] == 'edit') {
                                        $row = $proses->tampil_data_saja('*', 'izin_usaha', 'id = "' . $_GET['id'] . '"');
                                    ...
                                    <div class="row pt-2">
                                        <div class="col-xl-12 row">
                                            <div class="col-xxl-4 col-md-4 pt-2">
                                                ...
                                                <?= izin_usaha('jenis_izin', @$row['jenis_izin']); ?>
                                            </div>
                                            <div class="col-xxl-4 col-md-4 pt-2">
                                                <label for="basiInput" class="form-label">Nomor *</label>
                                                <input type="text" class="form-control" name="nomor" id="nomor" value="<?= @$row['nomor']; ?>" required>
                                            </div>
                                            ...
                                            <div class="col-xxl-8 col-md-8 pt-2">
                                                <label for="basiInput" class="form-label">Instansi Pemberi *</label>
                                                <input type="text" class="form-control" name="instansi_pemberi" value="<?= @$row['instansi_pemberi']; ?>" required>
                                            </div>
                                            <div class="col-xxl-4 col-md-4 pt-2">
                                                <label for="basiInput" class="form-label">Kualifikasi *</label>
    ...
    <script>
        ...
        function jenis_izin(jenis, kode) {
            if (jenis == 'Nomor-Induk-Berusaha-(NIB)') {
                $.ajax({
                    type: 'post',
                    url: '<?= $url; ?>form/form',
                    data: {
        ...
        $(document).on('change', '#jenis_izin', function(e) {
            var jenis = $('#jenis_izin').val();
            var kode = $('#nomor').val();
            jenis_izin(jenis, kode);
        });
        ...
        $(document).on('click', '#pilih', function(e) {
            ...
            var jenis = $('#jenis_izin').val();
            if (nomor == '') {
                alert('Nomor Izin Usaha Belum diisi!');
            } else {
                $.ajax({
                    type: 'post',
