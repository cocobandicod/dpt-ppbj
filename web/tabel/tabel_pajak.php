...
$sql = $proses->tampil_data_select('*', 'pajak', 'id_profil ="' . $_SESSION['kode_profil'] . '"');
...
foreach ($sql as $row) {
    ...
    $subdata = array();
    $subdata[] = $no;
    $subdata[] = str_replace('-', ' ', $row['jenis_laporan']);
    $subdata[] = $row['nomor'];
    $subdata[] = $row['tanggal'];
    $subdata[] = $status;
    $subdata[] = '
                <div class="dropdown d-inline-block">
                    ...
                    <li><a href="#" data-bs-toggle="modal" data-bs-target="#modalview" data-act="' . $act . '" data-id="' . $row['file'] . '" class="dropdown-item edit-item-btn"><i class="ri-search-eye-line align-bottom me-2 text-muted"></i> File</a></li>
                        <li><a href="' . $url . 'syarat/pajak/edit/' . $row['id'] . '" data-id="' . $row['id'] . '" class="dropdown-item edit-item-btn"><i class="ri-pencil-fill align-bottom me-2 text-muted"></i> Edit</a></li>
                        <li>
