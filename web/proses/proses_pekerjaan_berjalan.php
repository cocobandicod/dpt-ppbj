...
if (@$_POST['act'] == 'add') {
    if (!empty($_FILES['fupload']['tmp_name'])) {
        ...
        if (!($ekstensi_ok1)) {
            ...
            $result = $proses->tambah_data('pekerjaan_berjalan', $data); // SIMPAN KE DATABASE
        }
    }
    ...
    if (!empty($_FILES['fupload']['tmp_name'])) {
        ...
        if (!($ekstensi_ok1)) {
            ...
            $file = $proses->cek_fetch('pekerjaan_berjalan', 'id = "' . $_POST['id'] . '"');
            if (!empty($file['file'])) {
            ...
            $result = $proses->edit_data('pekerjaan_berjalan', $data, 'id', $_POST['id']); // SIMPAN KE DATABASE
        }
    } else {
    ...
    $id = $_POST['del'];
    $result = $proses->hapus_data('pekerjaan_berjalan', 'id', $id);
    exit();
}
