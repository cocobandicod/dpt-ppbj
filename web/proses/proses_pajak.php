...
if (@$_POST['act'] == 'add') {
    if (!empty($_FILES['fupload']['tmp_name'])) {
        ...
        if (!($ekstensi_ok1)) {
            ...
            $data[] = array(
                ...
                'jenis_laporan'     => strip_tags($_POST['jenis_laporan']),
                'file'              => $name1,
                'status'            => 'Pending'
            );
            $result = $proses->tambah_data('pajak', $data); // SIMPAN KE DATABASE
        }
    }
    ...
    if (!empty($_FILES['fupload']['tmp_name'])) {
        ...
        if (!($ekstensi_ok1)) {
            ...
            $file = $proses->cek_fetch('pajak', 'id = "' . $_POST['id'] . '"');
            if (!empty($file['file'])) {
            ...
            $result = $proses->edit_data('pajak', $data, 'id', $_POST['id']); // SIMPAN KE DATABASE
        }
    } else {
        $data = array(
            'nomor'             => strip_tags($_POST['nomor']),
    ...
    $id = $_POST['del'];
    $file = $proses->cek_fetch('pajak', 'id = "' . $id . '"');
    if (!empty($file['file'])) {
        unlink('../../berkas/' . $file['file']);
    }
    $result = $proses->hapus_data('pajak', 'id', $id);
    exit();
}
