...
if (@$_POST['act'] == 'add') {
    if (!empty($_FILES['fupload']['tmp_name'])) {
        ...
        if (!($ekstensi_ok1)) {
            ...
            $result = $proses->tambah_data('pengurus', $data); // SIMPAN KE DATABASE
        }
    }
    ...
    if (!empty($_FILES['fupload']['tmp_name'])) {
        ...
        if (!($ekstensi_ok1)) {
            ...
            $file = $proses->cek_fetch('pengurus', 'id = "' . $_POST['id'] . '"');
            if (!empty($file['file'])) {
            ...
            $result = $proses->edit_data('pengurus', $data, 'id', $_POST['id']); // SIMPAN KE DATABASE
        }
    } else {
        $data = array(
            'nomor_ktp'         => strip_tags($_POST['nomor_ktp']),
    ...
    $id = $_POST['del'];
    $file = $proses->cek_fetch('pengurus', 'id = "' . $id . '"');
    if (!empty($file['file'])) {
        unlink('../../berkas/' . $file['file']);
    }
    $result = $proses->hapus_data('pengurus', 'id', $id);
    exit();
}
