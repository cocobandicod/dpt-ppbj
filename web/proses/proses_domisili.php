...
if (@$_POST['act'] == 'add') {
    if (!empty($_FILES['fupload']['tmp_name'])) {
        ...
        if (!($ekstensi_ok1)) {
            ...
            $result = $proses->tambah_data('domisili', $data); // SIMPAN KE DATABASE
        }
    }
    ...
    if (!empty($_FILES['fupload']['tmp_name'])) {
        ...
        if (!($ekstensi_ok1)) {
            ...
            $file = $proses->cek_fetch('domisili', 'id = "' . $_POST['id'] . '"');
            if (!empty($file['file'])) {
            ...
            $result = $proses->edit_data('domisili', $data, 'id', $_POST['id']); // SIMPAN KE DATABASE
        }
    } else {
        $data = array(
            'nomor'             => strip_tags($_POST['nomor']),
    ...
    $id = $_POST['del'];
    $file = $proses->cek_fetch('domisili', 'id = "' . $id . '"');
    if (!empty($file['file'])) {
        unlink('../../berkas/' . $file['file']);
    }
    $result = $proses->hapus_data('domisili', 'id', $id);
    exit();
}
