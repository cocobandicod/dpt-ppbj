...
if (@$_POST['act'] == 'add') {
    if (!empty($_FILES['fupload']['tmp_name'])) {
        ...
        if (!($ekstensi_ok1)) {
            ...
            $result = $proses->tambah_data('npwp', $data); // SIMPAN KE DATABASE
        }
    }
    ...
    if (!empty($_FILES['fupload']['tmp_name'])) {
        ...
        if (!($ekstensi_ok1)) {
            ...
            $result = $proses->edit_data('npwp', $data, 'id', $_POST['id']); // SIMPAN KE DATABASE
        }
    } else {
        $data = array(
            'npwp'              => strip_tags($_POST['npwp'])
    ...
    $id = $_POST['del'];
    $file = $proses->cek_fetch('npwp', 'id = "' . $id . '"');
    if (!empty($file['file'])) {
        unlink('../../berkas/' . $file['file']);
    }
    $result = $proses->hapus_data('npwp', 'id', $id);
    exit();
}
